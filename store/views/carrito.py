...
def create_carrito(request, cliente_id):
    ...
    if cliente_id == 0:                                       # ← invitado
        ...
        # SEGURIDAD: Verificar que el usuario es quien dice ser (JWT o sesión)
        token_user_id, token_user_role = validate_jwt_token(request)
        session_cliente_id = request.session.get('cliente_id')
        
        # Solo admin, el propio usuario (JWT), o sesión válida pueden crear carrito
        if token_user_role != 'admin' and token_user_id != cliente_id and session_cliente_id != cliente_id:
            return JsonResponse({
                "error": "No autorizado",
                "detail": "Solo puedes agregar productos a tu propio carrito"
            }, status=403)
...
# ═════════════════════════════════════════════════════════════
# CHECKOUT PARA INVITADOS (Sin JWT)
# ═════════════════════════════════════════════════════════════

def checkout_guest_page(request):
    """
    Página de checkout para invitados.
    Muestra formulario para recolectar datos del cliente y finalizar compra.
    GET /checkout/guest/
    """
    carrito = get_carrito_by_session(request.session.session_key)
    ...
    for cp in carrito.items.select_related('variante__producto').all():
        var = cp.variante
        prod = var.producto
        precio = var.precio if var.precio else prod.precio
    ...
    return render(request, 'public/carrito/checkout_guest.html', {
        'carrito': carrito,
        'items': items,
        'total': float(total),
        'total_piezas': total_piezas,
...
@csrf_exempt
@require_http_methods(["POST"])
def finalizar_compra_guest(request):
    """
    Finaliza la compra para un invitado.
    POST /api/checkout/guest/finalizar/
    Body: { nombre, correo, telefono, direccion?, ... }
    """
    ...
    cliente, created = Cliente.objects.get_or_create(
        correo=correo,
        defaults={
            'username': correo.split('@')[0] + '_' + secrets.token_hex(4),
            'password': make_password(secrets.token_hex(16)),  # Contraseña aleatoria
            'nombre': nombre,
            'telefono': telefono,
            'direccion': direccion,
    ...
    # Si el cliente ya existía, actualizar datos
    if not created:
    ...
    for cp in carrito.items.select_related('variante__producto__categoria'):
        var = cp.variante
        prod = var.producto
        precio_unit = var.precio if var.precio else prod.precio
        subtotal = precio_unit * cp.cantidad
    ...
    if not orden:
        try:
            ...
            return JsonResponse({"error": f"Error al crear orden: {str(e)}"}, status=500)
    ...
    # Intentar enviar WhatsApp (no bloquea si falla)
    try:
        twilio_client = Client(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
        
        body_lines = [
            f"🧾 NUEVO PEDIDO #{carrito.id}",
            f"👤 Cliente: {cliente.nombre}",
            f"📧 Email: {cliente.correo}",
            f"📞 Tel: {cliente.telefono}",
            f"🏠 Dir: {cliente.direccion or 'No especificada'}",
            "",
            f"🛍️ Items: {total_piezas}",
            f"💰 Total: ${payload['total_amount']}",
            "",
            f"👉 Procesar: {link}"
        ]
        ...
        twilio_client.messages.create(
            from_=settings.TWILIO_WHATSAPP_FROM,
            to=settings.TWILIO_ADMIN_PHONE,
            body="\n".join(body_lines)
        )
    except Exception as e:
    ...
    # Si el cliente es nuevo, enviar email de verificación
    if created:
        try:
            from ..utils.security import send_verification_email
            send_verification_email(cliente, request)
        except Exception as e:

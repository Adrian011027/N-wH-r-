      ...
      <table class="variantes-table">
        ...
        <tbody>
          ${producto.variantes.map(v => {
            const tallasDisplay = v.tallas_stock 
              ? Object.entries(v.tallas_stock).map(([t, s]) => `${t}: ${s}`).join(', ')
              : '—';
            const stockTotal = v.stock_total || v.stock || 0;
            return `
            <tr>
              <td><span class="talla-badge">${v.color || '—'}</span></td>
              <td>${tallasDisplay}</td>
